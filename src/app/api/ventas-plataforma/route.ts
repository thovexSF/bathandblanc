...
import { getVentasPorPlataforma, Filtros } from '@/lib/queries';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const data = await getVentasPorPlataforma(filtros);
    
    return NextResponse.json(data);
  } catch (error) {
    console.error('Error obteniendo ventas por plataforma:', error);
